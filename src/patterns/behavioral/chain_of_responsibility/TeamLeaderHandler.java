...
/**
 * 团队负责人（TeamLeaderHandler）— 责任链中的第一个处理器。
 *
 * <h2>审批权限</h2>
 * <p>可审批 ≤3 天的请假申请。超出 3 天的请假需要传递给部门经理。</p>
 *
 */
public class TeamLeaderHandler extends Handler {
    ...
    /**
     * 处理请假请求：如果请假天数 ≤3 天则审批通过，否则传递给下一个处理器。
     *
    ...
    protected boolean process(LeaveRequest request) {
        if (request.getDays() <= MAX_DAYS) {
            System.out.println("【团队负责人】审批通过：同意 " + request.getApplicant()
                + " 请假 " + request.getDays() + " 天（原因：" + request.getReason() + "）。");
            return true;
        }
        System.out.println("【团队负责人】无法审批 " + request.getDays() + " 天的请假（最多审批 " + MAX_DAYS + " 天），转交上级...");
        return false;
    }
}
